...
export function SecureShieldCard({
  vehicleTitle = "Certified Vehicle",
  vehicleId,
  sellerPhone,
  onBookShield,
  isEnabled = true,
  ...
  const handlePaymentSuccess = (receipt: { transactionId: string; amount: number; method: string }) => {
    setIsAdded(true);
  ...
  return (
    <>
        ...
        >
          {isAdded ? (
            <>
              ...
              <span>🛡️ SECURE SHIELD ACTIVE (₹199)</span>
            </>
          ) : (
            <>
              <Award className="w-4 h-4 text-stone-950 shrink-0" />
              <span>🛡️ BOOK SECURE SHIELD VIA UPI (₹199)</span>
            </>
          )}
        </button>
      </div>
      ...
      <UPIPaymentModal
        ...
        amount={199}
        itemTitle={`Secure Shield Inspection: ${vehicleTitle}`}
        itemDescription="150-point physical inspection certification, RTO documentation & legal clearance guarantee."
      />
    </>
  );
}
