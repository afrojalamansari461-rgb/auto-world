...
export default function Navbar({ activeTab, setActiveTab, subscriptionActive, currentUser, onSignInClick }: NavbarProps) {
  ...
  return (
    <>
        ...
        <div className="max-w-[1650px] mx-auto px-4 sm:px-6 lg:px-10">
          <div className="flex justify-between items-center gap-2 lg:gap-6">
            ...
            <motion.button
              ...
              className="lg:hidden p-2.5 min-w-[44px] min-h-[44px] flex items-center justify-center rounded-full hover:bg-stone-200/60 text-stone-900 transition focus:outline-none focus-visible:ring-2 focus-visible:ring-stone-900 shrink-0"
            >
          ...
          {isMobileMenuOpen && (
            ...
            >
              {navItems.map((item) => {
                ...
                return (
                  <button
                    ...
                    aria-selected={isActive}
                    onClick={() => handleTabClick(item.id)}
                    className={`flex items-center gap-3 px-4 py-3 min-h-[44px] rounded-xl text-xs font-sans uppercase tracking-widest transition ${
                      isActive
              ...
              <div className="flex flex-col gap-2">
                {currentUser ? (
                  <div className="flex items-center justify-between p-3 bg-stone-100 rounded-xl border border-stone-200">
                    ...
                    <button
                      onClick={handleSignOut}
                      aria-label="Sign out of AutoWorld account"
                      className="flex items-center gap-1.5 px-3 py-2 min-h-[44px] bg-red-50 hover:bg-red-100 text-red-600 text-[11px] uppercase font-bold tracking-wider rounded-lg transition cursor-pointer"
                    >
                  ...
                  <button
                    ...
                    className="w-full flex items-center justify-center gap-2 py-3 px-5 min-h-[44px] bg-stone-900 text-white text-xs font-sans uppercase tracking-widest transition-all font-bold rounded-xl cursor-pointer"
                  >
                ...
                {subscriptionActive ? (
                  <div className="flex items-center justify-center gap-2 py-3 px-5 min-h-[44px] bg-stone-950 text-amber-400 text-xs tracking-widest uppercase font-bold rounded-xl border border-amber-500/30">
                  ...
                  <button
                    onClick={() => handleTabClick("premium")}
                    aria-label="Upgrade to AutoWorld Premium Plan"
                    className="w-full flex items-center justify-center gap-2 py-3 px-5 min-h-[44px] bg-amber-500/10 hover:bg-amber-500/20 border border-amber-600/30 text-amber-950 text-xs font-sans uppercase tracking-widest transition-all font-bold rounded-xl cursor-pointer"
                  >
      ...
      <div className="lg:hidden fixed bottom-0 left-0 right-0 z-[90] bg-[#FAF8F5]/95 backdrop-blur-md border-t border-stone-200/90 shadow-lg px-2 py-1.5 flex items-center justify-around">
        {navItems.slice(0, 5).map((item) => {
          ...
          return (
            <button
              key={`bottom-${item.id}`}
              onClick={() => handleTabClick(item.id)}
              className={`flex flex-col items-center justify-center min-w-[56px] min-h-[48px] py-1 px-1 rounded-lg text-[10px] font-bold uppercase transition ${
                isActive ? "text-stone-900 font-black scale-105" : "text-stone-500 hover:text-stone-800"
              }`}
            >
              <Icon className={`w-5 h-5 mb-0.5 ${isActive ? "text-amber-600" : "text-stone-500"}`} />
